...
data atl_precip;
	...
	*Precip is character, converts to numeric value;
	*TotalPrecip+Precip;
	if Precip ne 'T' then 
	 ...
	 *convert date column as numeric;
	 format Date date9.;
run;
...
*Numeric values have been converted to character values at the places;
*Z format write a number as a character string and insert
*leading zero to fill 5 total positions;
